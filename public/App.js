...
import { 
  Heart, Gift, Copy, Check, Loader2, QrCode, 
  Plus, Minus, Settings, Trash2, X, Pencil, Phone, Mail, Fingerprint, Hash, AlertTriangle
} from 'lucide-react';

// Configuração do Firebase integrada no ambiente
// FIX: Adicionado verificação para evitar crash se a variável não estiver definida
const firebaseConfig = typeof window.__firebase_config !== 'undefined' 
  ? JSON.parse(window.__firebase_config) 
  : { apiKey: "DEV_MODE", authDomain: "localhost" }; // Fallback seguro

const app = initializeApp(firebaseConfig);
const auth = getAuth(app);
const db = getFirestore(app);
const appId = typeof window.__app_id !== 'undefined' ? window.__app_id : 'wedding-gui-nuria';
...
export default function App() {
  ...
  const addItem = async (e) => {
    ...
    // FIX: Substituir vírgula por ponto para evitar erros de parsing
    const valorFloat = parseFloat(String(newItem.valor).replace(',', '.') || 0);
    
    await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'gifts', id), {
      ...newItem, id, valor: valorFloat
    });
  ...
  const handleUpdateItem = async (e) => {
    ...
    // FIX: Substituir vírgula por ponto para evitar erros de parsing
    const valorFloat = parseFloat(String(editingItem.valor).replace(',', '.') || 0);
    
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'gifts', editingItem.id), {
      ...editingItem, valor: valorFloat
    });
    setEditingItem(null);
  };
  ...
  const copyToClipboard = (text) => {
    // FIX: Uso da API moderna de clipboard
        ...
        <div className="min-h-screen flex items-center justify-center bg-stone-100 p-6 font-sans">
          <div className="bg-white p-10 rounded-[2.5rem] shadow-xl w-full max-w-md text-center border border-stone-200">
            ...
            <input 
              ...
              className="w-full border-b border-stone-200 focus:border-stone-900 outline-none py-4 mb-8 text-center text-2xl tracking-widest transition-all" 
              onKeyDown={e => e.key === 'Enter' && (passInput === ADMIN_PASSWORD ? setIsAdminAuthenticated(true) : alert("Incorreta!"))} 
            />
        ...
        <main className="max-w-7xl mx-auto p-6 md:p-10 grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-1 bg-white p-8 rounded-[2rem] shadow-sm border border-stone-100 h-fit">
            ...
            <form onSubmit={addItem} className="space-y-5">
              ...
              <input placeholder="Valor (R$)" type="text" inputMode="decimal" className="w-full p-4 bg-stone-50 rounded-xl outline-none border border-transparent focus:border-stone-100 transition-all" value={newItem.valor || ''} onChange={e => setNewItem({...newItem, valor: e.target.value})} required />
            ...
            <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl max-w-md w-full animate-in zoom-in duration-200 max-h-[90vh] overflow-y-auto text-center">
              ...
              <form onSubmit={handleUpdateItem} className="space-y-4">
                ...
                <div className="text-left space-y-1">
                  ...
                  <input type="text" inputMode="decimal" className="w-full p-4 bg-stone-50 rounded-xl outline-none border border-transparent focus:border-stone-200 transition-all" value={editingItem.valor || ''} onChange={e => setEditingItem({...editingItem, valor: e.target.value})} required />
                </div>
                <div className="text-left space-y-1">
